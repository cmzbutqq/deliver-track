...
本项目是电商物流配送场景的完整解决方案，涵盖商家端订单管理、用户端实时追踪、后端自动化轨迹模拟三大模块。系统通过 WebSocket 实时推送包裹位置，结合高德地图 API 进行真实路径规划和时间计算，使用基于时间数组的动态更新机制模拟配送过程，并提供了丰富的数据可视化看板。
...
- ✅ 智能路径规划（高德 API + 降级策略）
- ✅ 自动化轨迹模拟（基于时间数组的动态更新机制，1 秒 = 1 小时演示加速）
- ✅ 地理空间计算（射线法判断配送范围）
- ✅ 数据统计分析（总览、区域、物流公司维度）
...
- `prisma migrate reset --force` 会删除所有数据库数据，重新运行迁移和 seed，确保每次启动后数据库状态完全一致
- 按 `Ctrl+C` 停止服务后，会自动回到项目根目录
- 如果脚本没有执行权限，运行 `chmod +x start-backend.sh start-frontend.sh`
...
- 实时位置更新：WebSocket 基于时间数组动态推送位置，车辆图标使用缓动函数平滑移动
- 路径分段显示：已走过路径（深蓝实线）+ 未走过路径（浅灰虚线）
- 交通路况显示：支持实时显示高德地图交通路况图层，用户可一键开启/关闭路况显示
- 订单信息卡片：订单号、状态、进度百分比（基于时间计算）、物流公司、预计送达时间
...
**轨迹模拟**：
- 基于时间数组的动态更新：使用高德 API 返回的时间数据，结合物流公司 speed 系数和随机因子，计算真实配送时间
- 独立定时器机制：每个订单创建独立的定时器，基于 t_real 数组按时间顺序更新位置（1 秒 = 1 小时演示加速）
- 路径规划：优先使用高德地图 API 获取真实路径和时间数据（200+ 路径点），失败时降级到直线插值（20 个点）
...
**系统特性**：
- ✅ 完整业务闭环：商家创建订单 → 模拟发货 → 后端生成轨迹和时间数组 → 独立定时器推送位置 → 用户实时追踪 → 自动确认收货
- ✅ 智能路径规划：优先使用高德 API 获取真实路径（200+ 路径点），失败时自动降级到直线插值
- ✅ 交通路况显示：支持实时显示高德地图交通路况图层，用户可一键开启/关闭路况显示
...
**订单追踪页面没有更新**：
- **检查订单状态**：确保订单状态为 `SHIPPING`（运输中）
- **检查后端日志**：查看是否有 `🚀 启动订单 XXX 的配送定时器` 日志
- **检查 WebSocket 连接**：打开浏览器控制台，查看 WebSocket 连接状态和 `location_update` 事件
- **等待更新**：更新间隔基于时间数组动态计算，可能较长（1秒对应1小时演示时间）
